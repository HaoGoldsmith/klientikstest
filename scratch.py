...
#функция
def add_new_clients(client,bot_variables,phone,first_name,patron_name,second_name,email,living_address,sex,
                    appointment_confirmation_sms,lead_source,client_groups):
#добавить клиента
        try:
                ...
                if api_key:
                        ...
                        if not client_groups:
                            ...
                            if not client_groups:
                                return {'status': '0', 'error': 'Missing required variables - client_groups'}
                        if not living_address:
                            living_address = bot_variables.get('living_address')
                            if not living_address:
                                   return {'status': '0', 'error': 'Missing required variables - living_address'}
                        if not sex:
